...
export function PropertyTable({ properties, sort, order }: PropertyTableProps) {
  ...
  if (properties.length === 0) {
    return (
      <div className="glass-card rounded-3xl border border-white/40 p-12 text-center shadow-2xl shadow-brand-black/5">
          ...
          <tbody className="divide-y divide-gray-100/50">
            {properties.map((p) => (
              <tr
                ...
                className="hover:bg-white/60 transition-colors duration-300 group cursor-pointer"
              >
                <td className="px-4 py-3">
                  <span className="font-medium text-brand-black group-hover:text-brand-gold transition-colors">
                    {p.nama_property}
                  </span>
                </td>
